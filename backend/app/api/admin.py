...
async def avtonet_progressive_state(
    db: AsyncSession = Depends(get_db),
    _: User = Depends(require_admin),
) -> BolhaProgressiveState:
    from scraper.sources.avto_net_common import LOOKAHEAD_ADS
    ...
    r = await db.execute(
        text(
            ...
            SELECT MAX(CAST(external_id AS BIGINT))
            FROM listings
            WHERE source = 'avto.net' AND external_id ~ '^[0-9]+$'
            """
        )
    )
    db_max_raw = r.scalar_one_or_none()
    ...
    probes = (
        (await db.execute(select(AvtonetAdProbe).where(AvtonetAdProbe.ad_id.in_(all_ids))))
        .scalars()
        .all()
    )
    probe_by_id = {p.ad_id: p for p in probes}
    ...
    in_rows = (
        (
            await db.execute(
                select(AvtonetInactiveAd).where(AvtonetInactiveAd.ad_id.in_(all_ids))
            )
        )
        .scalars()
        .all()
    ...
    st_rows = (
        (await db.execute(select(AvtonetAdState).where(AvtonetAdState.ad_id.in_(all_ids))))
        .scalars()
        .all()
    )
    state_by_id = {r.ad_id: r for r in st_rows}

    def build_row(ad_id: int, *, zone: str) -> BolhaProgressiveRow:
        pr = probe_by_id.get(ad_id)
        inc = inactive_by_id.get(ad_id)
        st = state_by_id.get(ad_id)
        pipeline = st.status if st else None
    ...
    lookahead_rows = [build_row(i, zone="lookahead") for i in la_ids]
    pivot_row = build_row(pivot_id, zone="last_working" if lw > 0 else "anchor")
    tail_rows = [build_row(i, zone="tail") for i in tail_ids]
...
@router.get("/avtonet/ad-states", response_model=list[BolhaAdStateOut])
async def avtonet_ad_states(
    db: AsyncSession = Depends(get_db),
    _: User = Depends(require_admin),
    limit: int = Query(default=10_000, ge=1, le=50_000),
) -> list[AvtonetAdState]:
    stmt = select(AvtonetAdState).order_by(AvtonetAdState.ad_id.desc()).limit(limit)
    result = await db.execute(stmt)
    return list(result.scalars().all())
