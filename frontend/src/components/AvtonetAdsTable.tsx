...
function scrapeTooltip(s: AvtonetAdScrapeEntry): string {
  const parts = [
    fmtAt(s.at),
    fmtOffset(s.offset_seconds),
    s.result,
    s.source.replace('avto.net.', '').replace('avto.net', 'avto'),
  ]
    ...
    <Card>
      ...
      {q.error && (
        <p className="mb-4 text-sm text-red-600">
          Failed to load avtonet ads (is the latest migration applied?).
        </p>
      )}
      ...
      {!q.isLoading && q.data && q.data.length === 0 && (
        <p className="text-sm text-zinc-500">
          No rows in avtonet_ads yet — run <code className="text-zinc-500">make avtonet:lookahead</code>.
        </p>
      )}
                  ...
                  return (
                    <Fragment key={row.ad_id}>
                      <AdDataRow
                        ...
                        windowMs={windowMs}
                        onMatch={runMatch}
                        matchLoading={matchingAdId === row.ad_id && matchAvtonet.isPending}
