...
import {
  ...
  type AvtonetProgressiveState,
  type ScraperEvent,
  useAvtonetProgressiveState,
} from '../lib/admin'
...
type AvtonetProgressiveDashboardProps = {
  enabled: boolean
  liveEvents: ScraperEvent[]
}

export function AvtonetProgressiveDashboard({
  enabled,
  liveEvents,
}: AvtonetProgressiveDashboardProps) {
  const qc = useQueryClient()
  const progressive = useAvtonetProgressiveState(enabled)
  ...
  useEffect(() => {
    if (
      !last ||
      (last.kind !== 'avtonet_progress_tick' && last.kind !== 'avtonet_scout_done')
    ) {
      return
    }
    if (last.id === lastTickId.current) return
    lastTickId.current = last.id
    void qc.invalidateQueries({ queryKey: adminKeys.avtonetProgressive })
    void qc.invalidateQueries({ queryKey: adminKeys.avtonetAdStates })
  }, [last, qc])

  if (!enabled) return null

  const d: AvtonetProgressiveState | undefined = progressive.data
    ...
    <Card className="mb-6">
      <div className="mb-4 flex flex-col gap-2 border-b border-zinc-200 pb-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="text-right text-xs text-zinc-500">
          <p>
            Scan anchor max(high-water, db):{' '}
            <span className="font-mono text-zinc-700">{d?.scan_anchor_ad_id ?? '—'}</span>
          </p>
          <p>
            DB max: <span className="font-mono text-zinc-700">{d?.db_numeric_max ?? '—'}</span>
          </p>
          <p>
            LOOKAHEAD_ADS:{' '}
      ...
      {!d && !progressive.isLoading && (
        <p className="text-sm text-zinc-500">
          No progressive data yet. Run{' '}
          <code className="text-zinc-600">make avtonet:lookahead</code> with the worker connected.
        </p>
      )}
    </Card>
  )
