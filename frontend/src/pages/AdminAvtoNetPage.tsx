  ...
  return (
    <AdminSourceDebugScrape sourceName="avto.net" title="avto.net debug">
      <p>
        ...
        <code className="text-zinc-200">scraper/sources/avto_net.py</code> (wide-open filters,
        first page). avto.net is commonly protected by a Cloudflare-style edge; automated clients
        often get <strong className="text-zinc-200">403</strong> with no usable HTML.
      </p>
      <p>
        On a successful 200 HTML response, the parser extracts listing cards into{' '}
        <code className="text-zinc-200">ScrapedItem</code> rows and the worker upserts into Postgres
        (deduped by source + external id).
      </p>
      <p className="text-amber-200/90">
        Expect <code className="text-amber-100">403</code> until the fetch path is upgraded (e.g.
