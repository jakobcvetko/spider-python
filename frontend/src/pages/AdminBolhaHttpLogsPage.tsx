...
export default function AdminBolhaHttpLogsPage() {
  const live = useScraperLive(true)
  const rows = useBolhaHttpLogs(live.events)
  ...
  return (
    <>
      <header className="mb-4">
        <h1 className="text-2xl font-semibold tracking-tight">Bolha HTTP logs</h1>
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-3 py-8 text-center text-zinc-500">
                    {scraperConnected
                      ? 'Waiting for Bolha HTTP traffic…'
                      : 'Start a Bolha worker (lookahead or backfill) to see requests here.'}
                  </td>
                </tr>
              ) : (
                rows.map((ev) => {
