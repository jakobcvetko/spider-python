...
│   │   ├── worker.py              # entrypoint: `uv run python -m scraper.worker`
...
| `make dev` | Most common. Starts Postgres, runs migrations, then backend (`:8000`) + frontend (`:5173`) with prefixed logs. Ctrl+C cleans everything up via `scripts/dev.sh`. |
| `make scraper` | Run the scraper worker. **`make dev` does NOT start it.** |
...
```bash
cd backend
uv run python -m scraper.worker
